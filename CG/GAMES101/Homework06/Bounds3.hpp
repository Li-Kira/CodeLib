...
#else
    auto x_min = (pMin[0] - ray.origin[0]) * invDir[0];
    auto x_max = (pMax[0] - ray.origin[0]) * invDir[0];
    if (!dirIsNeg[0])
        std::swap(x_min, x_max);

    auto y_min = (pMin[1] - ray.origin[1]) * invDir[1];
    auto y_max = (pMax[1] - ray.origin[1]) * invDir[1];
    if (!dirIsNeg[1])
        std::swap(y_min, y_max);

    auto z_min = (pMin[2] - ray.origin[2]) * invDir[2];
    auto z_max = (pMax[2] - ray.origin[2]) * invDir[2];
    if (!dirIsNeg[2])
